...
const STATUS: Record<string, { label: string; bg: string; color: string }> = {
  approved: { label: "Succès",     bg: "#D1FAE5", color: "#059669" },
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center px-4 py-4 bg-white border-b border-gray-100">
        ...
        <h1 className="flex-1 text-center text-gray-900 text-base font-bold pr-8">Historique des retraits</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-4 space-y-3">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center py-20 text-center">
            ...
            <p className="text-gray-500 text-sm font-medium">Aucun retrait effectué</p>
            <p className="text-gray-400 text-xs mt-1">Vos retraits apparaîtront ici</p>
          </div>
        ) : (
          sorted.map(w => {
            const st = STATUS[w.status] || { label: w.status, bg: "#F3F4F6", color: "#6B7280" };
            ...
            return (
              ...
              >
                ...
                <div className="h-1 w-full rounded-t-2xl"
                  style={{ background: w.status === "approved" ? "#10b981" : w.status === "rejected" ? "#EF4444" : "#F59E0B" }} />

                <div className="px-4 py-4 space-y-2.5">
                  {/* Amount row */}
                  <div className="flex items-center justify-between">
                    <span className="text-gray-500 text-xs">Montant demandé</span>
                  ...
                  {net !== amount && (
                    <div className="flex items-center justify-between">
                      <span className="text-gray-500 text-xs">Montant reçu</span>
                      <span className="font-bold text-sm text-green-600">{fmt(net)} {currency}</span>
                    </div>
                  )}
                  ...
                  {w.paymentMethod && (
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-gray-700 text-xs font-medium">{w.paymentMethod}</span>
                    </div>
                  )}

                  {/* Status + date */}
                  <div className="flex items-center justify-between pt-1">
                    <span className="text-gray-400 text-xs">{fmtDate(w.createdAt)}</span>
                    <span

...
export default function AdminStaking() {
  ...
  const totalStaked = stakings.filter(s => s.status === "active").reduce((sum, s) => sum + parseFloat(String(s.amountPaid)), 0);
  const totalReleased = stakings.filter(s => s.status === "released").reduce((sum, s) => sum + parseFloat(String(s.returnAmount)), 0);

...
export const stakingProducts = pgTable("staking_products", {
  ...
  returnAmount: decimal("return_amount", { precision: 15, scale: 2 }).notNull(),
...
// Wheel of Fortune spins per user
export const wheelSpins = pgTable("wheel_spins", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id).unique(),
  spinsAvailable: integer("spins_available").notNull().default(0),
  totalSpinsUsed: integer("total_spins_used").notNull().default(0),
  productSpinClaimed: boolean("product_spin_claimed").notNull().default(false),
  referralSpinsClaimed: integer("referral_spins_claimed").notNull().default(0),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});

// Wheel spin history (prize log)
export const wheelSpinHistory = pgTable("wheel_spin_history", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id),
...
export type WheelSpins = typeof wheelSpins.$inferSelect;
export type WheelSpinHistory = typeof wheelSpinHistory.$inferSelect;

// Avis / Reviews (withdrawal proofs)
export const reviews = pgTable("reviews", {
  ...
  images: text("images").notNull().default("[]"), // JSON array of base64 strings
